...
uint8_t handleCommand(TIME *tm)
{
 ...
 // TIME tm;
  ...
  }else if(strcmp(cmd, "led1On") == 0)
  {
	  led1On();
  }else if(strcmp(cmd, "led1Off") == 0)
  {
	  led1Off();
  }else if(strcmp(cmd, "led2On") == 0)
  {
	  led2On();
...
void led2On(){
	...
	saveLedState(1);//ld2On
}
void led2Off(){
	HAL_GPIO_WritePin(LD2_GPIO_Port, LD2_Pin, 0);
	saveLedState(0);//ld2Off
}

void led1On(){
	HAL_GPIO_TogglePin(LD1_GPIO_Port, LD1_Pin);
	saveLedState(3);//ld1On
}
void led1Off(){
	HAL_GPIO_WritePin(LD1_GPIO_Port, LD1_Pin, 0);
	saveLedState(2);//ld2Off
}
