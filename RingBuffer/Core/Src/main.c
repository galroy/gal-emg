...
int UART_Transmit(UART_HandleTypeDef *huart, uint8_t *pData, size_t len)
{
	if (HAL_UART_Transmit_IT(huart, pData, len) != HAL_OK) {
		if (!RingBuffer_Write(&rb1,pData, len)) {
			return 0;
		}
...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart) {
	uint8_t txData;
	if (!RingBuffer_IsEmpty(&rb1)) {
		RingBuffer_Read(&rb1,&txData, 1);
...
int main(void)
{
  ...
  HAL_NVIC_EnableIRQ(USART2_IRQn);
	...
	if (RingBuffer_Init(&rb1) == RESULT_SUCCESS) {
//			char* data = "01234";
//			RingBuffer_Write(&rb1,data, 5);
		}
  printMessages2();
  //TestRingBuffer();
